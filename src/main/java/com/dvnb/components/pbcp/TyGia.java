...
//		tfNgayTyGia.setWidth("20%");
		...
		btnDel.setIcon(FontAwesome.SAVE);
		btnDel.setStyleName(ValoTheme.BUTTON_LARGE);
...
//			}
		...
		btnSave.addClickListener(event -> {
			...
			if(!tfTyGia.isEmpty()) {
				try {
					...
					dvnbTygia.setNgayTyGia(dateTyGia.isEmpty() ? null : new SimpleDateFormat("dd/MM/yyyy").format(dateTyGia.getValue()));
		...
		btnDel.addClickListener(event -> {
			if(!cbbKy.isEmpty() && !cbbCardbrn.isEmpty()) {
				String ky = cbbKy.getValue().toString();
				String cardBrn = cbbCardbrn.getValue().toString();
				tyGiaService.deleteByKyAndCrdBrn(ky, cardBrn);
				Notification.show("Đã xóa tỷ giá " + cardBrn + " kỳ " + ky, Type.WARNING_MESSAGE);
				refreshData();
			}
		...
		layoutButton.addComponent(btnDel);
	...
	private void refreshData() {
		...
		List<DvnbTyGia> listTygia = StreamSupport.stream(listTygiaIter.spliterator(), false).sorted((o1, o2)->o1.getCrdBrn().compareTo(o2.getCrdBrn())).collect(Collectors.toList());
		...
		if (!listTygia.isEmpty()) {
			...
			for (int i = 0; i <= listTygia.size() - 1; i++) {
				...
				item.getItemProperty("id").setValue(StringUtils.isNotEmpty(listTygia.get(i).getId()) ? listTygia.get(i).getId().toString() : "");
				item.getItemProperty("cretms").setValue(StringUtils.isNotEmpty(listTygia.get(i).getCreTms()) ? listTygia.get(i).getCreTms().toString() : "");
				item.getItemProperty("usrid").setValue(StringUtils.isNotEmpty(listTygia.get(i).getUsrId()) ? listTygia.get(i).getUsrId().toString() : "");
				item.getItemProperty("updtms").setValue(StringUtils.isNotEmpty(listTygia.get(i).getUpdTms()) ? listTygia.get(i).getUpdTms().toString() : "");
				item.getItemProperty("upduid").setValue(StringUtils.isNotEmpty(listTygia.get(i).getUpdUid()) ? listTygia.get(i).getUpdUid().toString() : "");
				item.getItemProperty("ky").setValue(StringUtils.isNotEmpty(listTygia.get(i).getKy()) ? listTygia.get(i).getKy().toString() : "");
				item.getItemProperty("crdbrn").setValue(StringUtils.isNotEmpty(listTygia.get(i).getKy()) ? listTygia.get(i).getCrdBrn().toString() : "");
				item.getItemProperty("ngayTyGia").setValue(StringUtils.isNotEmpty(listTygia.get(i).getNgayTyGia()) ? listTygia.get(i).getNgayTyGia().toString() : "");
